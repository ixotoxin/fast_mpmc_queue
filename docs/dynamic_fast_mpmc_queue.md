...
```c++
xtxn::dynamic_fast_mpmc_queue<payload_type> queue {};
```
`payload_type` must have a default constructor.

### Constructor of `dynamic_fast_mpmc_queue`
```c++
dynamic_fast_mpmc_queue::dynamic_fast_mpmc_queue();
```
...
#### Capacity
```c++
size_type dynamic_fast_mpmc_queue::capacity();
```
...
#### Free slots
```c++
size_type dynamic_fast_mpmc_queue::free_slots();
```
...
#### Emptiness
```c++
bool dynamic_fast_mpmc_queue::empty();
```
...
#### Producer slot
```c++
producer_accessor dynamic_fast_mpmc_queue::producer_slot(unsigned slot_acquire_attempts = A);
```
...
#### Consumer slot
```c++
consumer_accessor dynamic_fast_mpmc_queue::consumer_slot();
```
...
xtxn::dynamic_fast_mpmc_queue<int> queue {};
...
xtxn::dynamic_fast_mpmc_queue<payload_type, 64, 1024, false> queue {};
