...
```c++
xtxn::fastest_mpmc_queue<payload_type> queue {};
```
...
#### Producer slot
```c++
producer_accessor fastest_mpmc_queue::producer_slot(int32_t slot_acquire_attempts = c_default_attempts);
```
...
#### Consumer slot
```c++
consumer_accessor fastest_mpmc_queue::consumer_slot(int32_t slot_acquire_attempts = c_default_attempts);
```
...
```c++
xtxn::fastest_mpmc_queue<int> queue {};
